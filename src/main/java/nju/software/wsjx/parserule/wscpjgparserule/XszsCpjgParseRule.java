 ...
 *
 */
public class XszsCpjgParseRule extends GeneralCpjgCommonRule implements CpjgParseRule{
	
	private List<String> ayList;
	
	public XszsCpjgParseRule(){
		InputStream is = null;
...
//			}
		...
		//		setXsesjafs(cpjgModel, pjjgnr);//刑事二审结案方式
			setXszsjafs(cpjgModel,pjjgnr);//刑事再审结案方式
...
//			setJtfz(cpjgModel, pjjgnr);//是否集团犯罪
	...
	/**
	 * 改判原因
	 * @param cpjgModel 裁判结果模型
	 * @param wsAnalyse 文书分析
	 */
	 ...
	 * @param jgnr
	 */
	public void setXszsjafs(WscpjgModel cpjgModel, String jgnr) {
//		发回重审
...
//	String reg = "犯([\u4e00-\u9fa5]+?[、]*[\u4e00-\u9fa5]+?)*罪";
	...
	Matcher m_2 = p_2.matcher(pjjg);
	List<String> zmList = new ArrayList<String>();//所有罪名
	while(m.find()){
		zmList.add(m.group());
	}
	while(m_2.find()){
		zmList.add(m_2.group());
	}
...
//				A罪；B罪；A罪
		...
		if(/*zmList.size()>1 && i==zmList.size()-1 &&*/ StringUtil.contains(pfContent, "决定执行")){
			zxPfnr = pfContent.substring(pfContent.indexOf("决定执行"));
			pfContent = pfContent.substring(0, pfContent.indexOf("决定执行"));
		} 
...
//			原罪判罚-与前罪并罚
	...
	private String buildSplitExpression() {
		String expression = "";
