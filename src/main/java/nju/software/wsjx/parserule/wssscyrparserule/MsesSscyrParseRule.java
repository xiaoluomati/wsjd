 ...
 *
 */
public class MsesSscyrParseRule extends GeneralSscyrCommonRule implements SscyrParseRule{
...
//			}
			if(StringUtil.contains(ssdw, "上诉人")||StringUtil.contains(ssdw, "被上诉人")){
				wssscyrModel.setSsdw(ssdw);
			}
...
//			解析单位性质
	...
	public static void setFddbr(List<WssscyrModel> wssscyrModellist ){
		...
		for(int i=0;i<wssscyrModellist.size();i++){
			...
			if(!ssf && StringUtil.indexOf(ssdw, "上诉人")==0){
				ssfIndex = i;
				ssf = true;
			}
			if(!ysf && StringUtil.indexOf(ssdw, "被上诉人")==0){
				ysfIndex = i;
				ysf = true;
			}
		}
