 ...
 *
 */
public class XzesSscyrParseRule extends GeneralSscyrCommonRule implements SscyrParseRule{
	...
	public List<WssscyrModel> jxWssscyrModelList(WsAnalyse wsAnalyse) {
        ...
        for (int i = 0; i < sscyr.size(); i++) {
            ...
            //解析诉讼地位
            String ssdw=bssssf;
            if(StringUtil.contains(ssdw, "上诉人")||StringUtil.contains(ssdw, "被上诉人")||StringUtil.contains(ssdw,"被告")){
                wssscyrModel.setSsdw(ssdw);
                //解析行政法律关系主体
                if(StringUtil.equals(ssdw,"上诉人")){
                    wssscyrModel.setXzfagxzt("行政相对方");
                }else if(StringUtil.equals(ssdw,"被上诉人")){
            ...
            if(ysssdw!=null){
                ...
                if(ysssdw.contains("原审")){
                    if(ysssdw.indexOf("原审")+4<=ysssdw.length()) {
            ...
            String[] dlr={"法定代理人","委托代理人","法定代表人","责任人"};//代理人
            if(ysssdw!=null){
                if(ysssdw.equals("原告")){
                    dsrlb="原告";
                }else if(ysssdw.equals("被告")){
                    dsrlb="被告";
                }else if (ysssdw.equals("第三人")){
                    dsrlb="第三人";
                }
...
//			解析单位性质
    ...
    private void setXzglfw(WssscyrModel wssscyrModel) {
        ...
        String ysssdw=wssscyrModel.getYsssdw();//当事人原审诉讼地位
        if (dsrmc!=null&&dsrssdw!=null){
            if (StringUtil.equals(dsrssdw,"被上诉人")||StringUtil.equals(dsrssdw,"被告")||StringUtil.equals(ysssdw,"被告")) {
    ...
    public static void setDwxz(WssscyrModel wssscyrModel){
        ...
        }else if(name!=null && (name.endsWith("厂")||name.endsWith("商行")||name.endsWith("法律事务所")||name.endsWith("运输队")||name.endsWith("矿")||name.endsWith("养殖场"))){
    ...
    public static void setFddbr(List<WssscyrModel> wssscyrModellist ){
        ...
        boolean ysf = false;
        for(int i=0;i<wssscyrModellist.size();i++){
            String ssdw = wssscyrModellist.get(i).getSsdw();//诉讼身份
            if(ssdw!=null) {
                if (!ssf && ssdw.indexOf("上诉方") == 0) {
                    ssfIndex = i;
                    ssf = true;
                }
                if (ssdw != null) {
                    if (!ysf && ssdw.indexOf("被上诉人") == 0) {
                        ysfIndex = i;
                        ysf = true;
                    }
                }
